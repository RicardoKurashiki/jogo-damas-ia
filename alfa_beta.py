...
class AlphaBeta:
    ...
    def think(self, tableClass):
        ...
        if (type(move) is list):
            for m in move:
                print(
                    f"Máquina joga: {m.currentPos[0]},{m.currentPos[1]} >> {m.nextPos[0]},{m.nextPos[1]}")
        else:
            print(
                f"Máquina joga: {move.currentPos[0]},{move.currentPos[1]} >> {move.nextPos[0]},{move.nextPos[1]}")

        return move

    def minimax(self, tableClass, alpha, beta, depth, team):
        currentEvaluation = EvaluatedMove()
        currentEvaluation.evaluation = getEvaluation(
            tableClass.table, self.team)
        ...
        if self.team == team:
            ...
            for move in possibleMoves.movesBuffer:
                ...
                moveEval.evaluation = self.minimax(
                    myTable, alpha, beta, depth-1, enemyTeam).evaluation
...
def getEatCounter(table, context):
    ...
    availableMovements = getStoneAvailableMoves(
        currentPosition[0], currentPosition[1], myTable.table)

    while (availableMovements.hasEnemy == True and len(availableMovements.movesBuffer) > 0):
        counter += 1
        context = Context(
            currentPosition, availableMovements.movesBuffer[0], context.piece)
        ...
        if (currentLine > 0) and (currentColumn > 0):
            ...
            while ((currentLine - delta) > -1) and ((currentColumn - delta) > -1):
                ...
                if (upLeftPiece.team == enemy) and (upLeft[0] != 0) and (upLeft[1] != 0) and (Piece(table[currentLine - (delta+1)][currentColumn - (delta+1)]).team == Team.BLANK):
                    upLeft = [currentLine - (delta+1),
                              currentColumn - (delta+1)]
                    possibleEnemyMoves.append(upLeft)
                    break
                elif (upLeftPiece.team == Team.BLANK):
                    possibleFreeMoves.append(upLeft)
